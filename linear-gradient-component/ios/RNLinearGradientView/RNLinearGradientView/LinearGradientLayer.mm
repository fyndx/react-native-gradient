...
@implementation LinearGradientLayer
...
{
    // _colors = colors;
    _colors = [NSArray arrayWithObjects:(id)[[UIColor colorWithRed:34.0/255.0 green:211/255.0 blue:198/255.0 alpha:1.0] CGColor],(id)[[UIColor colorWithRed:145/255.0 green:72.0/255.0 blue:203/255.0 alpha:1.0] CGColor], nil];
...
+ (CGPoint) getGradientStartPointFromAngle:(CGFloat)angle AndSize:(CGSize)size
{
    ...
    if (fmodf(angle, 90) == 0)
        return [LinearGradientLayer getHorizontalOrVerticalStartPointFromAngle:angle AndSize:size];
    ...
    CGPoint startCorner = [LinearGradientLayer getStartCornerToIntersectFromAngle:angle AndSize:size];
...
- (void)drawInContext:(CGContextRef)ctx
{
    ...
    if (_useAngle)
    {
        ...
        CGPoint relativeStartPoint = [LinearGradientLayer getGradientStartPointFromAngle:angle AndSize:size];
